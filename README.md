...
netlify/functions/oku-fis.js   Anthropic API'ye görsel gönderip JSON döndüren serverless fonksiyon
...
- "Fotoğraf Çek" veya "Galeriden Seç" ile bir görsel seçildiğinde, görsel base64 olarak
  `/.netlify/functions/oku-fis` adresine POST edilir. Bu fonksiyon `ANTHROPIC_API_KEY` ortam
  değişkenini kullanarak Anthropic API'ye istek atar ve satıcı/tarih/tutar/KDV/kategori
  alanlarını JSON olarak döndürür. API anahtarı hiçbir zaman tarayıcıya gönderilmez.
...
4. Site ayarlarından **Environment variables** bölümüne gidin ve şunu ekleyin:
   - `ANTHROPIC_API_KEY` = Anthropic hesabınızdaki API anahtarı
   - (opsiyonel) `ANTHROPIC_MODEL` = kullanılacak model adı (varsayılan: `claude-sonnet-4-6`)
5. Deploy edin. `netlify/functions/oku-fis.js` otomatik olarak
...
kaldırır. Yerel `.env` dosyasına (repo'ya eklemeyin) `ANTHROPIC_API_KEY=...` satırını ekleyerek
...
- Uygulama tek kullanıcı için tasarlanmıştır, kimlik doğrulama içermez. Site adresini
  ...
  ekleyebilirsiniz.
- `ANTHROPIC_API_KEY` asla istemci koduna (JS/HTML) gömülmemelidir; yalnızca Netlify Function
